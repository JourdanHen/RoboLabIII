...
int posFinal = 175;  // Posição final dos servos
...
void processarEntrada(String entrada) {
  ...
  if (V > 127) {
    VD = map(V, 128, 254, VELOCIDADE_MIN, VELOCIDADE_MAX);
    VE = VD;
  } else if (V < 127) {
    VD = -map(V, 0, 126, VELOCIDADE_MAX, VELOCIDADE_MIN);
    VE = VD;
  } else {
    VD = 0;
    VE = 0;
  ...
  if (W > 127) {
    VD = VD - map(W, 128, 254, VELOCIDADE_MIN / 3, VELOCIDADE_MAX / 3);
    VE = VE + map(W, 128, 254, VELOCIDADE_MIN / 3, VELOCIDADE_MAX / 3);
  } else if (W < 127) {
    VD = VD + map(W, 0, 126, VELOCIDADE_MAX / 3, VELOCIDADE_MIN / 3);
    VE = VE - map(W, 0, 126, VELOCIDADE_MAX / 3, VELOCIDADE_MIN / 3);
  }
...
void loop() {
  ...
  if (currentConnectionStatus != lastConnectionStatus) {
    if (currentConnectionStatus) {
      ...
      servo_dir.write(posFinal);
      servo_esq.write(posFinal);
    } else {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    if (irValues[0] < 300) { // Esquerda frente
      ignoreBluetooth = true;
      V = 0;
      W = 254;
    }
    if (irValues[1] < 300) { // Esquerda trás
      ignoreBluetooth = true;
      V = 254;
      W = 254;
    }
    if (irValues[2] < 300) { // Direita frente
      ignoreBluetooth = true;
      V = 0;
      W = 0;
    }
    if (irValues[3] < 300) { // Direita trás
      ignoreBluetooth = true;
      V = 254;
      W = 0;
    }
    if (irValues[0] < 300 && irValues[2] < 300) { // Direita frente e Esquerda frente
      ignoreBluetooth = true;
      V = 0;
      W = 127;
    }
    if (irValues[1] < 300 && irValues[3] < 300) { // Direita trás e Esquerda trás
      ignoreBluetooth = true;
      V = 254;
      W = 127;
    }
    if (irValues[0] < 300 && irValues[1] < 300 && irValues[2] < 300 && irValues[3] < 300) { // Todos os sensores
      ignoreBluetooth = true;
      V = 127;
      W = 127;
    }
